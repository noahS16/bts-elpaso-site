...
function pinCity(place, autoOpen = false) {
    ...
    // If user selected a city, wait for flyTo animation to finish
    if (autoOpen) {
        map.flyTo([lat, lon], 6);
        createMarker();
    } else {
...
function cityPopupCard(data) {
    return `
    <div class="flex flex-row w-full items-center justify-between min-w-[150px] m-0">
        <div class="flex flex-col gap-0.5">
            <h3 class="text-2xl font-bold text-purple-600 m-0">${data.city}</h3>
            <span class="text-base">${data.country}</span>
        </div>
        <span class="text-end font-bold text-4xl text-red-500">${data.population.toLocaleString()}</span>
    </div>
  `;
}
